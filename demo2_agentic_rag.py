...
def expand_query(query: str) -> List[str]:
    """Expand the query with simple acronym or full-name variants."""
    variants = [query]
    q_lower = query.lower()
...
def semantic_search(
    ...
    """Retrieve the most relevant documents using semantic similarity only."""
    best_scores = np.full(doc_vectors.shape[0], -1e9, dtype=float)
    ...
    top_idx = np.argsort(best_scores)[::-1][:k]
    return [{"index": int(i), "score": float(best_scores[i])} for i in top_idx]
...
"""
    ...
    def retrieve_node(state: RAGState):
        ...
        hits = semantic_search(state["query"], emb, doc_vectors, k=5)
        return {"retrieved": hits}

    def grade_node(state: RAGState):
        """Check relevance: decide if the retrieved docs are strong enough."""
    ...
    def rewrite_node(state: RAGState):
        ...
        prompt = f"""
Rewrite this question into a short retrieval query.
